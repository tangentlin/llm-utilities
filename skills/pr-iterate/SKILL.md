...
Run both detections and use whichever succeeds.
...
If this returns a path, you're in a Sapling repo.
...
If this returns a path, you're in a Git repo.
...
With `GH_REPO` set, run all three fetches. **Use `--paginate`** on every call to ensure no comments are silently dropped (the default page size is 30).
